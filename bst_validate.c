...
bool is_bst_valid(Node* root, cir_queue* c_q)
{
    bool is_valid = true;
    if (!root)
        return true;
    ...
    while(!cir_queue_is_empty(c_q)) {
        ...
        if (right) {
            ...
            cir_queue_enqueue(c_q, right);
        }
    }
    return is_valid;
}

bool is_bst_valid_r(cir_queue* c_q)
{
    if (!c_q)
        return false;
...
int main(void)
{
    ...
    printf("\nCheck if BST is_bst_valid() %d\n", is_bst_valid(root, c_q));
    
    cir_queue_enqueue(c_q, root);
    printf("\nCheck if BST is_bst_valid_r() %d\n", is_bst_valid_r(c_q));
    return 0;
}